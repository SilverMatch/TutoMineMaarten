package com.silvermatch.advancedMod.world.gen;
...
public class WorldGeneratorFlag implements IWorldGenerator {
	...
	private WorldGenMinable flagGen = new WorldGenMinable(ModBlocks.frenchFlag, 32);
	@Override
	public void generate(Random random, int chunkX, int chunkZ, World world, IChunkProvider chunkGenerator, IChunkProvider chunkProvider) {
		LogHelper.info("Generating Flags");
	...
	private void generateSurface(World world, int x, int z, Random random) {
		if (random.nextInt(10)==0){
			...
			LogHelper.info("x:"+randX+" y:"+randY+" z:"+randZ);
		}
	...
	private void generateFlag(World world, int x, int y, int z){
		...
		for (int dx = 0 ; dx < 3 ; dx++){
			for (int dy = 0 ; dy < 2 ; dy++){
				world.setBlock(x+1+dx, y+9 - dy, z, ModBlocks.frenchFlag, 0, 2);
			}
		}
	}
}
